...
import { Calendar, User, Phone, CheckCircle2, Clock, MapPin } from 'lucide-react';
...
export default function ManualBookingModal({ isOpen, onClose, onSuccess }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!userName.trim()) { showToast('Ismni kiriting', 'warning'); return; }
    if (!userPhone.trim()) { showToast('Telefon raqamini kiriting', 'warning'); return; }
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          <label className="block text-xs font-semibold text-gray-500 mb-2 flex items-center gap-1.5">
            <MapPin size={14} /> MAYDONNI TANLANG
          </label>
